...
pub fn detect_utf16_pattern(bytes: &[u8]) -> Option<Encoding> {
    ...
    for (i, &b) in bytes.iter().enumerate() {
        if i % 2 == 0 {
            if b == 0 { even_null += 1; } else if (32..=126).contains(&b) { even_ascii += 1; }
        } else {
            if b == 0 { odd_null += 1; } else if (32..=126).contains(&b) { odd_ascii += 1; }
        }
    }
    ...
    if even_null_ratio > 0.85 && odd_ascii_ratio > 0.4 { return Some(Encoding::Utf16Le); }
    if odd_null_ratio > 0.85 && even_ascii_ratio > 0.4 { return Some(Encoding::Utf16Be); }
    None
}
